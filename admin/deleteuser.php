...
<body>
                    ...
                    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                      ...
                      $petqty = $_SESSION['petqty'];
                      if($petqty  > 0){
                       $q = "DELETE members, pet_info
                       FROM members
                       INNER JOIN pet_info ON members.member_id = pet_info.member_id
...
<?php
                    ...
                    echo '<form action="deleteuser.php?member='.$memberid.'" method="post">';
                    echo '<label style="font-size:18px; font-weight:bold;" class="text-danger mb-3"><i class="fas fa-trash-alt"></i>&nbsp;Delete A User</label>';
                    echo '<div class="alert alert-danger border-danger mb-4" role="alert">';
                    echo '<div><i class="fa fa-question-circle"></i>&nbsp;&nbsp;Are you sure to delete user <b>'; echo $row['username']; echo '</b> ?</div></div>';
                 ...
                 if ($numpet >= 1) {
                    ...
                    echo '<b>This user has '.$numpet.' pet(s). Deleting user would also result pet assigned to the user will be deleted !</b>';
                    echo '</div>';                
                    ...
                    echo '<div class="alert alert-info alert-dismissible fade show" role="alert">
                        <b>This user has no pet added to the system.</b></div>';
                 ...
                 <div class="col-md">
                   ...
                   <label class="form-check-label text-danger" for="yesChange">Yes, I want to delete <b><i>'.$row['username'].'</i></b> from the system.</label>
                   </div>
                 </div>
                 </div>';
                 ...
                 echo '</div>';
                  ...
                  echo '<button class="btn btn-danger btn-lg btn-block" type="submit" id="saveBtn" name="saveBtn" style="font-size:16px;" disabled ><i class="fas fa-trash-alt"></i>&nbsp; Delete user</button>';
